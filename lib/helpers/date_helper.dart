...
class DateHelper {
  ...
  static String getTimeAgo(int time) {
    ...
    if (diff < _minuteMillis) {
      ...
      return (diff / _minuteMillis).floor().toString() + " minutes ago";
    } else if (diff < 90 * _minuteMillis) {
      return "an hour ago";
    } else if (diff < 24 * _hourMillis) {
      return (diff / _hourMillis).floor().toString() + " hours ago";
  ...
  static String getShortTimeAgo(DateTime? dateTime) {
    ...
    if (difference.inDays > 29) {
      if ((difference.inDays / 30).floor() == 1) {
        ...
        time = "Active since " + (difference.inDays / 30).floor().toString() + "months";
      }
      ...
      if (difference.inHours == 1) {
        ...
        time = difference.inHours.toString() + " hours";
      }
    } else if (difference.inMinutes >= 1) {
      if (difference.inMinutes == 1) {
        time = "1 min";
      } else {
        time = difference.inMinutes.toString() + " min";
      }
    } else {
  ...
  static String getSinceAgo(DateTime dateTime) {
    ...
    if (difference.inDays > 29) {
      if ((difference.inDays / 30).floor() == 1)
        ...
        return "Active since " +
            (difference.inDays / 30).floor().toString() +
            "months";
    } else if (difference.inDays >= 1 && difference.inDays <= 29) {
      if (difference.inDays == 1)
